...
class pa2 extends State<Pagina3> {
  ...
  void enviarEliminar(int id) async {
    //int respuesta = await BdQR().eliminarProducto(codigo);
    bool? confirmacion = await showDialog(
      context: context,
      builder: (_context) => AlertDialog(
        title: Text("ATENCION"),
        content: Text("Deseas Eliminar la cuenta con el id:$id"),
        actions: [
          TextButton(
    ...
    if (confirmacion != null) {
      int respuesta = await Bdhelper().eliminarCuenta(id);
      if (respuesta != 0) {
        showDialog(
          context: context,
          builder: (context) {
              ...
              actions: [
                TextButton(
                  onPressed: () async {
                    ...
                    if (idd.isNotEmpty) {
                      await Bdhelper().eliminarCuenta(id);
